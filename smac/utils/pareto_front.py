...
def sort_by_hypervolume_contribution(
    ...
    # Normalize the costs per objective

    # Compute a reference point (with the local points or all observed history)

    # Apply reduce procedure

    # Sort based on HV contribution
...
def calculate_reference_point(
    ...
    config_instance_seed_budget_keys: list[list[InstanceSeedBudgetKey]] | None = None,
) -> list[float]:
    if configs is None:
        # Compute over the complete runhistory
        costs = [trail.cost for trial in runhistory.values()]
        return np.max(np.array(costs), axis=1)
    else:
        assert len(configs) == len(config_instance_seed_budget_keys)
    raise NotImplementedError