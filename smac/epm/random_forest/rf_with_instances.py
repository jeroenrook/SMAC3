...
class RandomForestWithInstances(BaseModel):
    ...
    def __init__(
        ...
        max_marginalized_instance_features: Optional[int] = np.inf,
    ) -> None:
        super().__init__(
            ...
            max_marginalized_instance_features=max_marginalized_instance_features,
        )
    ...
    def _train(self,  X: np.ndarray,  y: np.ndarray) -> "RandomForestWithInstances":
        ...
        leave_numbers = [len(t) for t in self.rf.all_leaf_values(X[0,:])]
        self.logger.debug(f"Random forest has {self.rf.num_trees()} trees which have respectively {leave_numbers} leaves.")
